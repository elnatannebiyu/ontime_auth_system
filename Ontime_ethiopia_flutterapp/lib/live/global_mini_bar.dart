...
class _GlobalMiniBarState extends State<GlobalMiniBar>
  ...
  Widget build(BuildContext context) {
    ...
    final showTv = tv.hasCurrent &&
        tv.playbackUrl != null &&
        tv.controller != null &&
        !tv.useUnifiedMiniPlayer &&
        !tv.inFullPlayer;
          ...
          children: [
            if (showTv) ...[
              ...
              Expanded(
                child: GestureDetector(
                  onTap: () {
                    ...
                    if (slug == null || slug.isEmpty) {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                            content: Text('Unable to open: missing channel')),
                      );
                      return;
                    }
                    final nav = appNavigatorKey.currentState;
                    if (nav == null) {
                      dev.log('Mini Tap: navigator not ready',
                          name: 'GlobalMini');
                      ScaffoldMessenger.of(context).showSnackBar(
                  ...
                  onVerticalDragUpdate: (details) {
                    setState(() {
                      _tvDragDy =
                          (_tvDragDy + details.delta.dy).clamp(-120.0, 0.0);
                    });
                  },
                  onVerticalDragEnd: (details) {
                    final upward = -_tvDragDy; // positive if dragged up
                    final velocityUp = (details.primaryVelocity ?? 0) <
                        -900; // fast upward fling
                  ...
                  child: Semantics(
                    button: true,
                    label:
                        'Mini TV preview. Drag up to expand. Double tap to open.',
              ...
              IconButton(
                ...
                icon:
                    Icon(tv.isPlaying ? Icons.pause_circle : Icons.play_circle),
                iconSize: 34,
