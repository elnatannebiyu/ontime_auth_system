...
class TvController extends ChangeNotifier {
  ...
  bool _wasPlaying = false; // whether playback was active when user navigated away
  ...
  void setCurrent({required String slug, required String title, String? sessionId, String? playbackUrl}) {
  ...
  Future<void> startPlayback({required String slug, required String title, required String url, String? sessionId}) async {
  ...
  Future<void> _startOrReuse(String url) async {
    if (_controller == null || _playbackUrl != url || !(_controller!.value.isInitialized)) {
...
class _MiniVideoBarState extends State<MiniVideoBar> {
            ...
            children: [
              ...
              Expanded(
                child: InkWell(
                  onTap: _openFull,
                  child: Text(
                    tv.title?.trim().isNotEmpty == true ? tv.title!.trim() : 'Live TV',
                    maxLines: 1,
  ...
  void _openFull() {
    ...
    Navigator.of(context).push(
      PageRouteBuilder(
        pageBuilder: (_, __, ___) => LivePlayerPage(slug: slug),
